...
class JobCreatorThread(threading.Thread):
        ...
        while not self.exitFlag:
            try:
                ...
                if syncs_changed:
                    ...
                    dirs = {
                        GENERATED_DIR: join(SHARED_DIR, v.get("APP"), GENERATED_DIR),
                        GENERATED_CONFIG_DIR: join(SHARED_DIR, v.get("APP"), GENERATED_CONFIG_DIR),
                        GENERATED_CATALOG_DIR: join(SHARED_DIR, v.get("APP"), GENERATED_CATALOG_DIR),
                    }
    ...
    def gen_dagster_job_archive(self, dirs: dict[str, str], syncs: Json[any]) -> None:
        ...
        shutil.make_archive(join(SHARED_DIR, f"{v.get('APP')}-valmi-jobs"), "zip", join(SHARED_DIR, v.get("APP")))
